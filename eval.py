...
if FLAGS.eval_train:
    x_raw, z_test, y_test = data_helpers.load_data_and_labels_dialog_act(FLAGS.positive_data_file)
...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        input_z = graph.get_operation_by_name("input_z").outputs[0]
        ...
        batches = data_helpers.batch_iter_da(list(zip(x_test, z_test)), FLAGS.batch_size, 1, shuffle=False)

        # Collect the predictions here
        all_predictions = []

        for batch in batches:
            x_batch, z_batch = zip(*batch)
            batch_predictions = sess.run(predictions, {input_x: x_batch, input_z: z_batch,dropout_keep_prob: 1.0})
